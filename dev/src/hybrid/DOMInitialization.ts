 ...
 * Type-safe references to all DOM elements
 */
export interface DOMElementRefs {
  // Buttons
  ...
  closeDashboardBtn: HTMLElement | null;

  // Template
  templateToggleBtn: HTMLButtonElement | null;
  templatePanel: HTMLElement | null;
  closeTemplatePanelBtn: HTMLElement | null;
...
function initializeElementReferences(): boolean {
  // Buttons
  refs.addTaskBtn = domManager.byId<HTMLButtonElement>('add-task-btn');
  refs.closeModalBtn = document.querySelector('.close-btn');
  refs.taskForm = domManager.byId<HTMLFormElement>('task-form');

  // Task form inputs
  refs.taskNameInput = domManager.byId<HTMLInputElement>('task-name');
  refs.estimatedTimeInput = domManager.byId<HTMLInputElement>('estimated-time');
  refs.actualTimeInput = domManager.byId<HTMLInputElement>('actual-time');
  refs.taskPriorityInput = domManager.byId<HTMLSelectElement>('task-priority');
  refs.taskCategoryInput = domManager.byId<HTMLSelectElement>('task-category');
  refs.taskDateInput = domManager.byId<HTMLInputElement>('task-date');
  refs.dueDateInput = domManager.byId<HTMLInputElement>('due-date');
  refs.dueTimePeriodInput = domManager.byId<HTMLSelectElement>('due-time-period');
  refs.dueHourInput = domManager.byId<HTMLSelectElement>('due-hour');
  refs.taskDetailsInput = domManager.byId<HTMLTextAreaElement>('task-details');

  // Recurrence elements
  refs.isRecurringCheckbox = domManager.byId<HTMLInputElement>('is-recurring');
  refs.recurrenceOptions = document.getElementById('recurrence-options');
  refs.recurrencePatternSelect = domManager.byId<HTMLSelectElement>('recurrence-pattern');
  refs.recurrenceEndDateInput = domManager.byId<HTMLInputElement>('recurrence-end-date');

  // Duplicate/Template buttons
  refs.duplicateTaskBtn = domManager.byId<HTMLButtonElement>('duplicate-task-btn');
  refs.saveAsTemplateBtn = domManager.byId<HTMLButtonElement>('save-as-template-btn');

  // Week navigation
  refs.prevWeekBtn = domManager.byId<HTMLButtonElement>('prev-week');
  refs.todayBtn = domManager.byId<HTMLButtonElement>('today');
  refs.nextWeekBtn = domManager.byId<HTMLButtonElement>('next-week');
  refs.datePicker = domManager.byId<HTMLInputElement>('date-picker');
  refs.weekTitle = domManager.byId<HTMLElement>('week-title');

  // Day columns
  const dayColumnElements = document.querySelectorAll('#task-board .day-column');
  refs.dayColumns = Array.from(dayColumnElements).filter(el => el instanceof HTMLElement) as HTMLElement[];

  refs.unassignedColumn = domManager.byId<HTMLElement>('unassigned-tasks');
  refs.unassignedList = domManager.byId<HTMLElement>('unassigned-list');

  // Settings
  refs.categoryFilterSelect = domManager.byId<HTMLSelectElement>('filter-category');
  refs.idealDailyMinutesInput = domManager.byId<HTMLInputElement>('ideal-daily-minutes');
  refs.weekdayFilterBtn = domManager.byId<HTMLButtonElement>('weekday-filter-btn');
  refs.weekdaySettings = document.getElementById('weekday-settings');

  // Theme
  refs.themeToggleBtn = domManager.byId<HTMLButtonElement>('theme-toggle');

  // More menu
  refs.moreMenuBtn = domManager.byId<HTMLButtonElement>('more-menu-btn');
  refs.moreMenuDropdown = document.getElementById('more-menu-dropdown');

  // Export/Import
  refs.exportDataBtn = domManager.byId<HTMLButtonElement>('export-data-btn');
  refs.importDataBtn = domManager.byId<HTMLButtonElement>('import-data-btn');
  refs.importFileInput = domManager.byId<HTMLInputElement>('import-file-input');

  // Archive
  refs.archiveToggleBtn = domManager.byId<HTMLButtonElement>('archive-toggle');
  refs.archiveView = domManager.byId<HTMLElement>('archive-view');
  refs.closeArchiveBtn = domManager.byId<HTMLButtonElement>('close-archive');
  refs.clearArchiveBtn = domManager.byId<HTMLButtonElement>('clear-archive');
  refs.archiveList = domManager.byId<HTMLElement>('archive-list');

  // Statistics
  refs.statisticsToggleBtn = domManager.byId<HTMLButtonElement>('statistics-toggle');
  refs.dashboardPanel = domManager.byId<HTMLElement>('dashboard-panel');
  refs.closeDashboardBtn = document.getElementById('close-dashboard');

  // Template
  refs.templateToggleBtn = domManager.byId<HTMLButtonElement>('template-toggle');
  refs.templatePanel = domManager.byId<HTMLElement>('template-panel');
  refs.closeTemplatePanelBtn = document.getElementById('close-template-panel');
  refs.templateSearchInput = domManager.byId<HTMLInputElement>('template-search');
  refs.templateSortSelect = domManager.byId<HTMLSelectElement>('template-sort');
  refs.templateList = domManager.byId<HTMLElement>('template-list');
 ...
 */
export function initializeEventListeners(): boolean {
  let success = true;
...
function initializeWeekNavigationListeners(): boolean {
  ...
  refs.datePicker?.addEventListener('change', (e: Event) => {
    if (refs.datePicker) {
...
function initializeWeekdayListeners(): boolean {
  ...
  document.addEventListener('click', (e: MouseEvent) => {
    if (refs.weekdaySettings && refs.weekdaySettings.contains(e.target as Node)) {
      return;
    }
    if (refs.weekdaySettings && !refs.weekdaySettings.contains(e.target as Node)) {
...
function initializeSettingsListeners(): boolean {
  ...
  refs.idealDailyMinutesInput?.addEventListener('change', (e: Event) => {
    const newValue = parseInt((e.target as HTMLInputElement).value) || 480;
...
function initializeMoreMenuListeners(): boolean {
  ...
  refs.moreMenuBtn?.addEventListener('click', (e: MouseEvent) => {
    e.stopPropagation();
    toggleMoreMenu();
  });
  ...
  // Close handler - click outside closes
  document.addEventListener('click', (e: MouseEvent) => {
    if (refs.moreMenuDropdown && !refs.moreMenuDropdown.contains(e.target as Node)) {
...
function initializeUnassignedListeners(): boolean {
  ...
  refs.unassignedColumn.addEventListener('click', (e: MouseEvent) => {
    e.preventDefault();
    openTaskModal();
  });
 ...
 */
export function getDOMRefs(): Readonly<DOMElementRefs> {
  return { ...refs };
}
 ...
 */
export function getRef<K extends keyof DOMElementRefs>(key: K): DOMElementRefs[K] {
  return refs[key];
}

// Expose to window for use by existing script.js
(window as any).HybridDOMInitialization = {
