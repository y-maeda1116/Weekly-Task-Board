/**
 * Outlook Connector - Comprehensive Unit Tests
...
describe("OutlookConnector - Unit Tests", () => {
  ...
  describe("OAuth Flow", () => {
    it("should construct with client ID and redirect URI", () => {
      const testConnector = new OutlookConnectorImpl("my-client-id", "http://example.com/callback");
    ...
    it("should initiate OAuth flow with correct parameters", async () => {
      ...
      try {
        ...
        expect(url.hostname).toContain("login.microsoftonline.com");
  ...
  describe("API Calls", () => {
    ...
    it("should throw error when not authenticated for getEvents", async () => {
      const newConnector = new OutlookConnectorImpl("client-id", "redirect-uri");
    ...
    it("should fetch event details by ID", async () => {
      ...
      expect(event.subject).toBe("Meeting");
    });
  ...
  describe("Edge Cases", () => {
    it("should handle empty client ID", () => {
      const testConnector = new OutlookConnectorImpl("", "http://localhost:3000/callback");
      expect(testConnector).toBeDefined();
    });

    it("should handle very long redirect URI", () => {
      const longUri = "http://localhost:3000/" + "a".repeat(1000);
      const testConnector = new OutlookConnectorImpl("client-id", longUri);
      expect(testConnector).toBeDefined();
    });
