import type { JournalEntry, DayJournal } from '../types/journal';
...
export function createEntry(taskId: string, taskName: string): JournalEntry {
...
export function completeEntry(entryId: string, nextStep: string): void {
  const journals = loadAll();
  for (const day of journals) {
...
export function updateJournal(entryId: string, text: string): void {
  const journals = loadAll();
  for (const day of journals) {
    const entry = day.entries.find(e => e.id === entryId);
    if (entry) {
...
export function getActiveEntry(): JournalEntry | null {
  const journals = loadAll();
  for (const day of journals) {
    for (const entry of day.entries) {
      if (entry.completedAt === null) {
...
export function getEntryByTaskId(taskId: string): JournalEntry | null {
  const journals = loadAll();
  for (const day of journals) {
