 ...
 */
function formatDueDate(date, timePeriod, hour) {
...
function handleTaskToggle(taskId) {
    if (window.HybridTaskOperations && window.HybridTaskOperations.toggleTaskCompletion) {
        window.HybridTaskOperations.toggleTaskCompletion(taskId);
        // Trigger re-render
...
function handleTaskEdit(taskId) {
    if (window.HybridTaskModal && window.HybridTaskModal.openEditModal) {
        window.HybridTaskModal.openEditModal(taskId);
...
function handleTaskDelete(taskId) {
    if (confirm('このタスクを削除しますか？')) {
        if (window.HybridTaskOperations && window.HybridTaskOperations.deleteTask) {
            window.HybridTaskOperations.deleteTask(taskId);
            // Trigger re-render
            if (window.renderWeek) {
                window.renderWeek();
            }
 ...
 */
function renderTasksToContainer(tasks, container) {
 ...
 */
function renderTasksForDate(tasks, date) {
 ...
 * Update daily time display
 */
function updateDailyTimeDisplay(column, tasks) {
    const timeSpan = column.querySelector('.daily-total-time');
    if (!timeSpan) {
        return;
    }
...
function renderWeek(tasks) {
    ...
    dayColumns.forEach(column => {
        ...
        if (date) {
            ...
            const tasksForDate = window.HybridTaskFiltering?.getTasksForDate
                ? window.HybridTaskFiltering.getTasksForDate(tasks, date)
                : tasks.filter((t) => t.assigned_date === date || (t.assigned_date === null && t.date === date));
    ...
    if (unassignedList) {
        const unassignedTasks = window.HybridTaskFiltering?.getUnassignedTasks
            ? window.HybridTaskFiltering.getUnassignedTasks(tasks)
            : tasks.filter((t) => t.assigned_date === null);
...
function handleDrop(event) {
    ...
    if (date) {
        ...
        if (window.HybridTaskOperations?.moveTask) {
            window.HybridTaskOperations.moveTask(taskId, date);
            // Trigger re-render
            if (window.renderWeek) {
                window.renderWeek();
            }
        ...
        if (window.HybridTaskOperations?.moveTaskToUnassigned) {
            window.HybridTaskOperations.moveTaskToUnassigned(taskId);
            // Trigger re-render
            if (window.renderWeek) {
                window.renderWeek();
            }
