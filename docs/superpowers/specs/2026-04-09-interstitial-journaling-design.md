...
- 実行中のタスクは同時に1つのみ。既に別タスクが実行中の場合は確認ダイアログを表示し、「現在のタスクを完了して新しいタスクを開始」「キャンセル」の2択とする

### タスク完了時の Next Step ダイアログ

- 既存の完了トグル操作時に、そのタスクに未完了の `JournalEntry` があれば:
  1. Next Step 入力ダイアログを表示
  2. ユーザーがNext Stepを入力して「保存」を押した場合:
     - `completedAt = now` を記録、Next Step を保存
     - タスクを完了状態に更新
  3. ユーザーが「スキップ」を押した場合:
     - `completedAt = now` のみ記録（Next Step は空文字）
     - タスクを完了状態に更新
